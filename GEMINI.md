...
I'm using mise to manage my Go environment.
...
```bash
# Rebuild the simpleicons.go file after updating simple-icons
./original-assets/build/buildIcons.sh
```

The social icons are sourced from the [Simple Icons](https://simpleicons.org/) project. They are compiled into the `simpleicons.go` file. To update the icons, you can run the `buildIcons.sh` script. This requires Node.js and npm.
